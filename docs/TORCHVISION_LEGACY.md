# Archived TorchVision backend documentation

This is the approximate legacy port, **not** the default original-source backend. Add `--backend torchvision` to all tool commands below. See the root README for strict BACL training.
...
Class-balanced repeating is optional (`--repeat-threshold 0.001`); it is disabled by default. The original BACL R50 LVIS v1 experiment configs use plain LVIS datasets, not ClassBalancedDataset.
